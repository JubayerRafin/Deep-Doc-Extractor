...
Each record preserves the Stage 2 schema plus a Stage 3 provenance extension:
{
  "messages": [
    {"role": "system",    "content": "..."},
...
def _build_record(messages: List[Dict], provenance: Dict,
    ...
    return {"messages": messages, "provenance": prov}
...
def export_augmented_dataset(
    ...
    final schema (status='augmented', augmented_from set, etc).
    """
    exporter = FinalExporter(config)
    # Ensure every original has a stage3 block
    finalised_originals = []
