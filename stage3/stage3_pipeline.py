...
from final_exporter import FinalExporter, _question_id, _build_record
...
def run_pipeline(config: Dict, dry_run: bool = False,
    ...
    if dry_run:
        ...
        originals = [_build_record(r["messages"], r.get("provenance", {}),
                                   "original", None, 0) for r in records]
    ...
    for idx, rec in enumerate(records):
        ...
        for _, reason in dropped:
            # Normalise reason to a short bucket for the stats counter
            bucket = reason.split("(")[0].strip()
            dropped_reasons[bucket] += 1
