...
def chunk_all(classified_blocks: List[tuple], config: Dict) -> List[Chunk]:
    ...
    for block, cat in classified_blocks:
        if block.block_type == "table" and block.table_data:
            ...
            b2 = ContentBlock(block.heading, tbl_text, "table", [], block.table_data, block.page_hint, block.source_file)
...
if __name__ == "__main__":
    ...
    for c in chunks[:5]:
        print(f"  [{c.chunk_index}] {c.category:12s} | {c.heading[:30]:30s} | {c.text[:60].replace(chr(10),' ')}...")
